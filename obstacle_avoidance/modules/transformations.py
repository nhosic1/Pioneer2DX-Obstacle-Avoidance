...
def get_camera_R(R01, R02):
    R_transf = np.mat([[0., 0., 1.], [-1., 0., 0.], [0., -1., 0.]])
    R01 = np.mat(R01)*R_transf
    R02 = np.mat(R02)*R_transf
    R12 = (np.mat(R01).T)*np.mat(R02)
    return R12

def get_camera_R_alt(R):
...
def get_camera_T(T):
    R_transf = np.mat([[0., 0., 1.], [-1., 0., 0.], [0., -1., 0.]])
    T_cam = R_transf.T*np.mat(T)
    return T_cam
    
def get_camera_T_alt(T):
    t1 = -T[1]
    t2 = -T[2]
    t3 = T[0]
    T = np.array([t1, t2, t3])
