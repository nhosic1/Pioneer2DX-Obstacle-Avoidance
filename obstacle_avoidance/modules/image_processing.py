...
def FLANN_matcher(kp1, kp2, des1, des2, detector):
    ...
    #     if np.linalg.norm(np.array(pnts1[i]) - np.array(pnts2[i])) > 5:
    #         mask.append(True)
    #     else:
    #         mask.append(False)
...
def KLT_algorithm(old_image, new_image, mask):
    ...
    p0_reverse, st, err = cv2.calcOpticalFlowPyrLK(frame_gray, old_gray, p1, None, **lk_params)
    d = abs(p0-p0_reverse).reshape(-1, 2).max(-1)
    good = d < 1 
    for i in range(len(good)):
        if good[i] == True and abs(p1[i] - p0[i]).max() < 10:
            good[i] = False

    # d_matches = abs(np.array(p1)-np.array(p0)).reshape(-1, 2).max(-1)
    # mask2 = d_matches > 10
    # good = mask1 & mask2
...
def find_matches(frame1, frame2, detector):
    ...
    if detector == None:
        # frame1 = cv2.Canny(frame1,20,50)
        # frame2 = cv2.Canny(frame2,20,50)
        frame1 = cv2.Canny(frame1,10,30)
        frame2 = cv2.Canny(frame2,10,30)
        pnts1, pnts2 = KLT_algorithm(frame1, frame2, None)
    else:
        # frame1 = cv2.Canny(frame1,10,40)
        # frame2 = cv2.Canny(frame2,10,40)
